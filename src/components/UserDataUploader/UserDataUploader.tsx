...
import { uploadAvatar } from '@/feauters/uploadAvatar'
...
export default function UserDataUploader({ isLogged, userFullname }: UserDataUploaderProps): React.JSX.Element {
    const [avatar, setAvatar] = useState<File | null>(null),
        [fullName, setFullName] = useState<string>(""),
    ...
    useEffect(() => {
        if (isLogged) {
            ...
            try {
                ...
                console.error('[USER_DATA_UPLOADER]: Ошибка подгрузки данных пользователя', error)
            }
        }
    }, [isLogged, userFullname])
    ...
    return (
        <div className="user-card">
            ...
            <h1 className="user-card_username">{fullName ? fullName : "Ошибка загрузки данных.."}</h1>
        </div>
    )
}
