import { Formalization_Test } from '@/constants/data'
import React, { useEffect, useState } from 'react'
import "./FormalizationTest.css"
...
const FormalizationTest: React.FC = () => {
  ...
  const handleAnswerOptionClick = async (isCorrect: boolean) => {
    ...
    if (nextQuestion < Formalization_Test.length) {
    ...
    <div className='true-and-false'>
      {showScore ? (
        <div className='score-section'>
          Верно {score} из {Formalization_Test.length}
          <br />
        ...
        <>
          <div className='question-section'>
            <div className='question-count'>
              <span>Вопрос {currentQuestion + 1}</span>/{Formalization_Test.length}
            </div>
            <div className='question-text'>{Formalization_Test[currentQuestion].question}</div>
          </div>
          <div className='answer-section'>
            {Formalization_Test[currentQuestion].options.map((option, index) => (
...
export default FormalizationTest
