import { TemporalOperators_Test } from '@/constants/data'
import React, { useEffect, useState } from 'react'
import "./OperatorsTest.css"
...
const TemporalOperators: React.FC = () => {
  ...
  const handleAnswerOptionClick = async (isCorrect: boolean) => {
    ...
    if (nextQuestion < TemporalOperators_Test.length) {
    ...
    <div className='true-and-false'>
      {showScore ? (
        <div className='score-section'>
          Верно {score} из {TemporalOperators_Test.length}
          <br />
        ...
        <>
          <div className='question-section'>
            <div className='question-count'>
              <span>Вопрос {currentQuestion + 1}</span>/{TemporalOperators_Test.length}
            </div>
            <div className='question-text'>{TemporalOperators_Test[currentQuestion].question}</div>
          </div>
          <div className='answer-section'>
            {TemporalOperators_Test[currentQuestion].options.map((option, index) => (
