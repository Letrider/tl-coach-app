import { LinearTemporalLogic_Test } from '@/constants/data'
import React, { useEffect, useState } from 'react'
import "./LinearTemporalTest.css"
...
const LinearTemporalLogic: React.FC = () => {
  ...
  const handleAnswerOptionClick = async (isCorrect: boolean) => {
    ...
    if (nextQuestion < LinearTemporalLogic_Test.length) {
    ...
    <div className='true-and-false'>
      {showScore ? (
        <div className='score-section'>
          Верно {score} из {LinearTemporalLogic_Test.length}
          <br />
        ...
        <>
          <div className='question-section'>
            <div className='question-count'>
              <span>Вопрос {currentQuestion + 1}</span>/{LinearTemporalLogic_Test.length}
            </div>
            <div className='question-text'>{LinearTemporalLogic_Test[currentQuestion].question}</div>
          </div>
          <div className='answer-section'>
            {LinearTemporalLogic_Test[currentQuestion].options.map((option, index) => (
...
export default LinearTemporalLogic
